...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-2.5">
            ...
            <div class="relative w-full sm:w-60">
                ...
                <input type="text" id="crop-search" placeholder="Search crops..." class="w-full pl-9 pr-4 py-2 text-sm bg-slate-900 border border-slate-800 rounded-lg text-slate-100 placeholder-slate-505 focus:outline-none focus:border-emerald-500/50 transition-colors">
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="crop-cards-container">
        @foreach($crops as $crop)
            <div class="glass-card glass-card-hover rounded-2xl overflow-hidden flex flex-col h-full crop-card" 
                ...
                <div class="h-44 w-full relative overflow-hidden bg-slate-900">
                    @if($crop->image_url)
                        <img src="{{ $crop->image_url }}" alt="{{ $crop->name }}" class="w-full h-full object-cover opacity-90 transition-transform duration-500 hover:scale-105">
                    @else
                        <div class="w-full h-full flex items-center justify-center bg-slate-900 text-slate-650">
                            No Image Available
                        </div>
                    @endif
                    <div class="absolute top-3 left-3 bg-emerald-500/90 text-slate-900 text-xs font-bold px-2.5 py-1 rounded-md uppercase tracking-wider">
                        {{ $crop->type }}
                    </div>
                </div>
                ...
                <div class="p-6 flex flex-col flex-grow space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-2 text-xs py-3 border-y border-slate-905">
                    ...
                    <div class="pt-2 flex justify-between items-center mt-auto">
                        <span class="text-xs font-semibold text-emerald-450">NPK: {{ $crop->optimal_n }}:{{ $crop->optimal_p }}:{{ $crop->optimal_k }}</span>
...
<div id="crop-modal" class="fixed inset-0 z-50 hidden bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="glass-card rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl relative">
        ...
        <div class="p-6 space-y-5">
            <div class="space-y-1">
                <span id="modal-crop-type" class="text-[10px] font-bold uppercase tracking-wider text-emerald-450 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">Cereal</span>
                <h3 id="modal-crop-name" class="text-2xl font-bold text-white mt-1.5">Crop Name</h3>
            </div>
            
            <p id="modal-crop-desc" class="text-xs text-slate-350 leading-relaxed">Description goes here...</p>

            <div class="grid grid-cols-2 gap-3 text-xs bg-slate-900/60 p-4 rounded-xl border border-slate-900">
                <div>
                    <span class="text-slate-550 block">Temperature Range</span>
                    <span id="modal-crop-temp" class="text-slate-200 font-semibold">-</span>
                </div>
                <div>
                    <span class="text-slate-550 block">Soil pH Range</span>
                    <span id="modal-crop-ph" class="text-slate-200 font-semibold">-</span>
                </div>
                <div class="mt-2">
                    <span class="text-slate-550 block">Watering Profile</span>
                    <span id="modal-crop-water" class="text-slate-200 font-semibold">-</span>
                </div>
                <div class="mt-2">
                    <span class="text-slate-550 block">Growth Period</span>
                ...
                <div class="col-span-2 mt-2 pt-2 border-t border-slate-800">
                    <span class="text-slate-550 block">Optimal N-P-K Ratio (kg/ha)</span>
                    <span id="modal-crop-npk" class="text-emerald-400 font-extrabold">-</span>
                </div>
                <div class="col-span-2 mt-2">
                    <span class="text-slate-550 block">Suitable Soils</span>
                    <span id="modal-crop-soils" class="text-slate-300 font-semibold">-</span>
                </div>
            </div>

            <div class="flex justify-end pt-2">
                <button onclick="closeCropDetails()" class="btn-premium-outline px-4 py-2 text-xs font-semibold rounded-lg">
                    Close Details
                </button>
            </div>
        </div>
