...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div class="space-y-2">
            ...
            <h3 class="text-2xl sm:text-3xl font-extrabold text-white">
                Smart Crop Directory
            </h3>

            <p class="text-sm text-slate-400">
                Search and filter crop varieties suitable for your climate and soil type.
            </p>
        </div>
        ...
        <div class="flex flex-wrap gap-2.5">
            ...
            <div class="relative w-full sm:w-60">
                ...
                <input
                    ...
                    class="w-full pl-9 pr-4 py-2 text-sm bg-slate-900 border border-slate-800 rounded-lg text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500/50 transition-colors"
                >
            </div>
            ...
            <select
                id="crop-type-filter"
                class="px-3 py-2 text-sm bg-slate-900 border border-slate-800 rounded-lg text-slate-300 focus:outline-none focus:border-emerald-500/50"
            >
    ...
    <div
        ...
        id="crop-cards-container"
    >

        @foreach($crops as $crop)

            <div
                class="glass-card glass-card-hover rounded-2xl overflow-hidden flex flex-col h-full crop-card"
                data-name="{{ strtolower($crop->name) }}"
                data-type="{{ $crop->type }}"
                data-soils="{{ json_encode(array_map('strtolower', $crop->soil_types)) }}"
                data-description="{{ strtolower($crop->description) }}"
            >
                ...
                <div class="p-6 flex flex-col flex-grow space-y-4">
                    ...
                    <div class="flex items-center justify-between">
                        <span class="bg-emerald-500/90 text-slate-900 text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wider">
                            {{ $crop->type }}
                        </span>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 text-xs py-3 border-y border-slate-800">
                        ...
                        <div class="space-y-0.5">
                            ...
                            <span class="text-slate-200 font-medium">
                                {{ $crop->optimal_temp_min }}°C -
                                {{ $crop->optimal_temp_max }}°C
                            </span>
                        </div>
                        ...
                        <div class="space-y-0.5 mt-2">
                            ...
                            <span
                                class="text-slate-200 font-medium truncate inline-block max-w-full"
                                title="{{ $crop->water_requirement }}"
                            >
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex flex-wrap gap-1">
                            ...
                            @foreach($crop->soil_types as $s)
                                <span class="bg-slate-900 text-slate-300 text-[10px] px-2 py-0.5 rounded border border-slate-800">
                                    {{ $s }}
                                </span>
                            @endforeach
                    ...
                    <div class="pt-2 flex justify-between items-center mt-auto">
                        ...
                        <button
                            onclick="openCropDetails(
                                '{{ $crop->name }}',
                                '{{ $crop->type }}',
                                '{{ addslashes($crop->description) }}',
                                '{{ $crop->optimal_temp_min }}°C to {{ $crop->optimal_temp_max }}°C',
                                '{{ $crop->optimal_ph_min }} to {{ $crop->optimal_ph_max }}',
                                '{{ $crop->harvest_days }} Days',
                                '{{ $crop->water_requirement }}',
                                '{{ $crop->optimal_n }}:{{ $crop->optimal_p }}:{{ $crop->optimal_k }}',
                                '{{ implode(', ', $crop->soil_types) }}'
                            )"
                            class="btn-premium-outline px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center space-x-1"
                        >
    ...
    <div
        id="no-crops-found"
        class="hidden text-center py-12 glass-card rounded-2xl border border-slate-900"
    >
...
>
    ...
    <div class="glass-card rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl relative">
        ...
        <button
            ...
            class="absolute top-4 right-4 p-1.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-white transition-colors"
        >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                    stroke-linecap="round"
        ...
        <div class="p-6 space-y-5">
            ...
            <div class="space-y-1">
                ...
                <span
                    id="modal-crop-type"
                    class="text-[10px] font-bold uppercase tracking-wider text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20"
                >
                    Cereal
                </span>
                ...
                <h3
                    id="modal-crop-name"
                    class="text-2xl font-bold text-white mt-1.5"
                >
                    Crop Name
                </h3>
            ...
            <div class="grid grid-cols-2 gap-3 text-xs bg-slate-900/60 p-4 rounded-xl border border-slate-800">
                ...
                <div class="col-span-2 mt-2 pt-2 border-t border-slate-800">
                    <span class="text-slate-500 block">
                        Optimal N-P-K Ratio (kg/ha)
                    </span>

                    <span id="modal-crop-npk" class="text-emerald-400 font-extrabold">
                        -
                    </span>
                </div>
                ...
                <div class="col-span-2 mt-2">
                    ...
                    <span id="modal-crop-soils" class="text-slate-300 font-semibold">
                        -
                    </span>
                </div>
            ...
            <div class="flex justify-end pt-2">
                ...
                <button
                    onclick="closeCropDetails()"
                    class="btn-premium-outline px-4 py-2 text-xs font-semibold rounded-lg"
                >
                    Close Details
                </button>
...
<script>
    ...
    function filterCrops() {
        ...
        cropCards.forEach(card => {
            ...
            const matchSoil =
                !selectedSoil || soils.includes(selectedSoil);
        ...
        if (visibleCount === 0) {
            noCrops.classList.remove('hidden');
        } else {
            noCrops.classList.add('hidden');
        }
    }
